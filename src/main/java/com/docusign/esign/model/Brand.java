...
/**
 * Information about a brand that is associated with an account. A brand applies custom styles and
 * text to an envelope..
 */
@Schema(
    description =
...
public class Brand {
  ...
  /**
   * An object containing the URLs for the four DocuSign master resource files that the brand uses
   * for sending, signing, email messages, and captive (embedded) signing. You can modify the
   * default email messages and formats in these file to customize the user experience. **Note:**
   * This object is returned only when the `resourceContentType` is `sending`, `signing`, `email`,
   * or `signing_captive`..
   *
   * @return resources
   */
