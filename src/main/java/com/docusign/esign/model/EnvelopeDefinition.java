...
public class EnvelopeDefinition {
  ...
  /**
   * This optional complex element enables the sender to override some envelope email setting
   * information. This can be used to override the Reply To email address and name associated with
   * the envelope, as well as the BCC email addresses to which an envelope is sent. When the
   * `emailSettings` information is used for an envelope, it only applies to that envelope.
   * **Important Note:** The `emailSettings` information is not returned in the GET method for
  ...
  /**
   * This optional object is used to register a webhook that will receive status changes for this
   * envelope..
   *
